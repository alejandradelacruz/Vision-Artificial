...
def loadImage(numero):
    __path__ = 'C:\\Users\\User\\Desktop\\VA\\Vision-Artificial\\Parcial2\\img'
    img_path = f"{__path__}\\img{numero}.jpg"
...
def showImages(original, cleaned_image, labeled_array, numero_imagen, obj_areas, obj_perimeters, obj_centroids, object_labels, object_names):
    ...
    colores[:, :2] = 0  # Fijar canales azul y verde a 0 (azul)
    colores[:, 2] = 255  # Fijar canal rojo a 255 (rojo) 
    ...
    for label_number in range(1, labeled_array.max() + 1):
        ...
        cv2.putText(imagen_coloreada, f'{label_number}', centroid,
...
def predict_by_feature_comparison(detected_features, reference_features, object_names):
    ...
    for detected in detected_features:
        ...
        object_predictions.append(
            object_names.get(predicted_label, 'Desconocido'))
...
def classify_objects(object_names):
    ...
    while True:
        ...
        # Si es la imagen 100 o superior, asigna 'Desconocido'
        if numero_imagen > 100:            
            object_predictions = ['Desconocido']
            print("El objeto es desconocido esta fuera del contexto")
            exit()
        ...
        if imagen is not None:
            ...
            for prediction in object_predictions:
                if prediction not in object_names.values():
...
def searchObjects(obj_type, object_names):
    ...
    for images in obj_images:
        print(images)
        image_number = f"{images}"
        img_path = f'C:\\Users\\User\\Desktop\\VA\\Vision-Artificial\\Parcial2\\img\\img{image_number}.jpg'
        print(img_path)
...
def searchObjects_classified(max_item, object_names):
    ...
    for i in range(104):
        ...
        if imagen is not None:
                ...
                # if obj_types.find(object_predictions):
                #     obj_images.append(i)

    # print(type(obj_types))
    # print(type(object_predictions))
    if i >= 102:
        print("Objeto desconocido")
    else:
        print("\n", elementos_extraidos)
...
def main():
    ...
    directorio = 'C:\\Users\\User\\Desktop\\VA\\Vision-Artificial\\Parcial2\\img'

    # Recorre todas las imágenes en el directorio
    for i, nombre_archivo in enumerate(os.listdir(directorio)):
       if nombre_archivo.endswith('.jpg'):  # Filtra por extensión (puedes cambiarla)
           nuevo_nombre = f'img{i+1}.jpg'
